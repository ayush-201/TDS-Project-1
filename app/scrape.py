...
SAVE_DIR = "discourse_posts"  # New directory for individual posts
...
def scrape_posts(playwright):
            ...
            for post in posts:
                ...
                safe_title = "".join(
                    [c if c.isalnum() or c in " _-()" else "_" 
                     for c in (post_data["topic_title"] or "untitled")]
                )
                filename = f"{safe_title}_post_{post_data['post_id']}.json"
    ...
    print(f"✅ Saved {total_saved} individual posts between {DATE_FROM.date()} and {DATE_TO.date()}")
    browser.close()
